...
def check(image, sample, target, direction):
    if direction == 'v':
        if ((target[sample[0, 0].long(), sample[0, 1].long() + 1] == 1) and (target[sample[0, 0].long(), sample[0, 1].long() - 1] == 0)): #left ,x small
            ...
            inner = np.array([]).reshape([0, 0])
            outer = np.array([]).reshape([0, 0])
...
def epecheck(mask, target, vposes, hposes):
    ...
    for idx in range(vposes.shape[0]):
        ...
        if v_in_site.shape[0] > 0:
            vioMap[v_in_site[:, 0].long(), v_in_site[:, 1].long()] = 1
        if v_out_site.shape[0] > 0:
...
class EPEChecker:
    @staticmethod
    def check(print_image : torch.tensor, target : torch.tensor):
        print_image = func.pad(print_image, (MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH), "constant", 0)
        target = func.pad(target, (MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH, MIN_EPE_CHECK_LENGTH), "constant", 0) 
        vposes, hposes = boundaries(target)
        epeIn, epeOut, _ =  epecheck(print_image, target, vposes, hposes)
        return epeIn + epeOut